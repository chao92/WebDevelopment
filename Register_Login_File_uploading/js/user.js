...
User = {
	 ...
	 * user name validator
	 */
	validInstitution: function(input)
	{
		var institution = input.value;
		if(institution.trim()=="") { return this.markInvalid(input, "You forgot your user name."); }
		if(institution.indexOf("'")>-1) { return this.markInvalid(input, "Apostrophes are not allowed in institution names."); }
		if(institution.length<2) { return this.markInvalid(input, "Sorry, user names must be more than 2 letters."); }
		return this.markValid(input);
	},
	...
	processRegistration: function()
	{
		...
		valid &= this.validInstitution(form["Institution"]);
	...
	processLogin: function()
	{
		...
		if(valid) {
			form["sha1"].value = form["password1"].value;
			//alert(form["sha1"].value);
			//form["password1"].value = this.randomString(16);
			form.submit(); }
	},
	...
	processUpdate: function()
	{
		...
		if(form["password1"].value.trim()!="") {
			valid &= this.passwordMatch(form["password1"], form["password2"]);
			//valid &= this.strongPassword(form["password1"]);
			if (!valid) {alert('invalid password');}
			if(valid) {
				form["sha1"].value = form["password1"].value;
				//form["password1"].value = this.randomString(16);
				//form["password2"].value = form["password1"].value;
