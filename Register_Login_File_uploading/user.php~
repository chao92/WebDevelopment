<?php
	...
	class User
	{
		...
		// =======================================================================
			...
			const DOMAIN_NAME = "humangenomeprivacy.org";

			// This is going to be the "from" address
			const MAILER_NAME = "noreply@humangenomeprivacy.org";

			// if you want people to be able to reply to a real address, override
			// this variable to "yourmail@somedomain.ext" here.
			const MAILER_REPLYTO = "shw070@ucsd.edu";

			const mail_cc_shaung = "shw070@ucsd.edu";
			const mail_cc_jiang = "x1jiang@ucsd.edu";
			//const mail_cc = "cjiang@ucsd.edu";
		...
		function rebuild_database($dbfile)
		{
			...
			$create = "CREATE TABLE users (username TEXT UNIQUE, firstname Text, lastname Text, password TEXT, email TEXT UNIQUE, token TEXT, role TEXT, active TEXT, last TEXT, institution Text, country Text, fileversion INTEGER, tasks TEXT);";
		...
		function process(&$registration_callback=false)
		{
			...
			if(isset($_POST["op"]))
			{
				/*echo "<script type='text/javascript'>alert('with operation');</script>";*/
				$operation = $_POST["op"];
				/*echo "<script type='text/javascript'>alert('<?php echo $operation; ?>');</script>";*/
				...
				elseif($operation == "validateTask") { $this->authenticated = $this->validateTask(); }
			...
			$this->token = $this->get_user_token($this->username);


			$this->institution = $this->get_user_institution($this->username);
			$this->fileversion = $this->get_user_fileversion($this->username);
	...
	// ---------------------
		...
		function login()
		{
			...
			$sha1 = $_POST["sha1"];
			/*echo "<script type='text/javascript'>alert ('<?php echo $sha1; ?>');</script>";*/
			...
			if(!$this->validate_user_name($username)) {
				...
				/*echo "<script type='text/javascript'>alert('validete username');</script>";*/
				return false; }
			/*if(preg_match(User::sha1regexp, $sha1)==0) {
				echo "<script type='text/javascript'>alert('validate password');</script>";
		...
		function validateTask()
		{
			...
			if(!empty($_POST['task'])){
				...
				foreach($_POST['task'] as $task_id){
        			/*echo "<script type='text/javascript'>alert('<?php echo $task_id; ?>');</script>";*/
        			$tasks[] = $task_id;
     			}
     			$username = $_SESSION["username"];
     			$tasklists = implode(",", $tasks);
     			/*echo "<script type='text/javascript'>alert('<?php echo $tasklists; ?>');</script>";*/
     			//$update="";
				//$update="UPDATE users SET tasks='$tasklists' WHERE username='$username'";
				//echo $update;
				//$updatetask = $this->database->exec($update); 

				$_SESSION["tasks"] = $tasklists;
				//print_r($this->database->errorInfo());
				//echo "result".$updatetask;
				return true;
			}else{
		...
		function logout()
		{
			...
			if(!$this->validate_user_name($username)) {
				$this->info("log in error: user name did not pass validation");
				$_SESSION["error_message"] = "log in error: user name did not pass validation";
				return false; }
		...
		function register(&$registration_callback=false)
		{
			...
			//$_SESSION["temp_Username"] =  $username;
/*			
echo "<script type='text/javascript'>alert ('<?php echo $username.",".$email.",".$sha1; ?>');</script>";*/
			// step 1: someone could have bypassed the javascript validation, so validate again.
			/*if(!$this->validate_user_name($username)) {
			...
			{
				$registered = false;
				$_SESSION["error_message"] = "Username and password are the same";
			}else{
				$registered = $this->register_user($username, $firstname, $lastname, $email, $institution, $country, $sha1, $registration_callback);
				$_SESSION["error_message"] = $this->error;
			}
			...
			if($registered && User::use_mail)
			{
				...
				$cc_shuang = User::mail_cc_shaung;
				$cc_jiang = User::mail_cc_jiang;
				$domain_name = User::DOMAIN_NAME;
				$subject = User::DOMAIN_NAME . " registration";
				$body = <<<EOT
Hi "$username",

This is a confirmation that you have registered at $domain_name with the user name: "$username" and email: $email.

If you were not the one to register this account, please contact us through email.

Of course, if you did register it by yourself, welcome to $domain_name!

The iDASH genome privacy protection committee.
EOT;
				$headers = "From: $from\r\n";
				$headers .= "CC: $cc_shuang,$cc_jiang,jiangchao19920119@gmail.com\r\n";
				//$headers .= "CC: jiangchao19920119@gmail.com\r\n";
		...
		function reset_password()
		{
			...
			$dbpassword = $sha1; //$this->token_hash_password($username, $sha1, $token);
...
EOT;
	...
	// ------------------
		...
		function authenticate_user($username, $operation)
		{
			...
			//echo "session token is :".$_SESSION["token"]."<br>";
			//echo  "db token is :".$token;
		 ...
		 */
		function register_user($username, $firstname, $lastname, $email, $institution, $country, $sha1, &$registration_callback = false)
		{
			$dbpassword = $sha1; //$this->token_hash_password($username, $sha1, "");
			...
			/*else{	$query = "SELECT username FROM users";
				...
				if(in_array($this->homogenise_username($username), $usernames)) {
					...
					echo "<script type='text/javascript'>alert ('because it is too similar to other user names');</script>";
					return false; }}*/

			// Is email address already in use? (see notes on safe reporting)
			/*echo "<script type='text/javascript'>alert('<?php echo $dbpassword; ?>');</script>";*/

			/*CREATE TABLE users (username TEXT UNIQUE, firstname Text, lastname Text, password TEXT, email TEXT UNIQUE, token TEXT, role TEXT, active TEXT, last TEXT, institution Text, country Text, fileversion INTEGER);"*/
			...
			$insert .= "VALUES ('$username', '$firstname', '$lastname', '$email', '$dbpassword', '', 'user', 'true', '" . time() . "','$institution', '$country', 1) ";
		...
		function login_user($username, $sha1)
		{
			...
			$dbpassword = $sha1; //$this->token_hash_password($username, $sha1, $this->get_user_token($username));
			/*if($dbpassword==$sha1) {

				$this->info("password hashing is not implemented.");
				$_SESSION["error_message"] = "password hashing is not implemented.";
				return false; }
*/
			...
			foreach($this->database->query($query) as $data) {
				...
				$_SESSION["error_message"] = "password mismatch for $username";
				if(User::unsafe_reporting) { $this->error = "incorrect password for $username."; $_SESSION["error_message"] = $this->error ;}
				else { $this->error = "the specified username/password combination is incorrect."; $_SESSION["error_message"] = $this->error ;}
				return false; }

			// authentication could not take place
			$this->info("there was no user $username in the database");
			if(User::unsafe_reporting) { $this->error = "user $username is unknown."; $_SESSION["error_message"] = $this->error ;}
			else { $this->error = "you either did not correctly input your username, or password (... or both)."; $_SESSION["error_message"] = $this->error ;}
			return false;
		}
		 ...
		 */
		function get_user_institution($username)
		{
			$query = "SELECT institution FROM users WHERE username = '$username'";
			foreach($this->database->query($query) as $data) { return $data["institution"]; }
			return false;
		}
		 ...
		 */
		function get_user_fileversion($username)
		{
			$query = "SELECT fileversion FROM users WHERE username = '$username'";
			foreach($this->database->query($query) as $data) { return $data["fileversion"]; }
			return false;
		}
