...
class PhpUploader
{
	...
	public $ProgressPanelWidth=510;
	
	/// The height of the Progress bar. Default is 20.
	public $ProgressBarHeight=50;
	...
	function PhpUploader($username, $fileversion, $email, $tasks)
	{
		...
		/*echo "<script type='text/javascript'>alert('<?php echo $this->username; ?>');</script>";*/
	...
	function SaveSecuritySettings()
	{
		...
		$this->_SaveSecuritySetting("username");
		$this->_SaveSecuritySetting("fileversion");
		$this->_SaveSecuritySetting("email");
		$this->_SaveSecuritySetting("tasks");
	}
	function LoadSecuritySettings()
	{
		...
		$this->_LaveSecuritySetting("username");
		$this->_LaveSecuritySetting("fileversion");
		$this->_LaveSecuritySetting("tasks");
		/*echo "<script type='text/javascript'>alert('<?php echo $this->username; ?>');</script>";*/
		...
		$currentversion = $this->fileversion;
		...
		foreach($conn->query($query) as $data){
			$current =$data["fileversion"]; 
		}*/
		/*echo "<script type='text/javascript'>alert('<?php echo $tasklists; ?>');</script>";*/
		$currentversion = $currentversion + 1;
      	$update = "UPDATE users SET fileversion = '$currentversion' WHERE username = '$username'";
      	$conn->exec($update);
      	//$conn->commit();


      	$update2 = "UPDATE users SET tasks = '$tasklists' WHERE username = '$username'";
      	$conn->exec($update2);
      	$conn->commit();
		/*
		if($current != 0){
			$current = $current + 1;
      		$update = "UPDATE users SET fileversion = '$current' WHERE username= '$username'";
      		echo "<script type='text/javascript'>alert('<?php echo 'here'; ?>');</script>";
      		$conn->exec($update);
      		$conn->commit();
		}*/
...
/*
	function get_user_fileversion($conn, $username)
	{
		$query = "SELECT fileversion FROM users WHERE username = '$username'";
		foreach($conn->query($query) as $data) { return $data["fileversion"]; }
		return false;
	}
*/	
	...
	function WriteValidationOK($message)
	{
		...
		// send confirmation email to uploaders
		if(!@$_SESSION)session_start();
		$this->_LaveSecuritySetting("username");
		$this->_LaveSecuritySetting("email");
		$this->_LaveSecuritySetting("fileversion");
		$username = $this->username;
		$email = $this->email;
		$fileversion = $this->fileversion;
		$from = "noreply@humangenomeprivacy.org";
		$replyto = "wsasyz@gmail.com";
		$domain_name = "humangenomeprivacy.org";
...
EOT;
		$headers = "From: $from\r\n";
		$headers .= "CC: wsasyz@gmail.com,xiaoqian.jiang@gmail.com,jiangchao19920119@gmail.com, diyuebu@gmail.com \r\n";
		$headers .= "Reply-To: $replyto\r\n";
		$headers .= "X-Mailer: PHP/" . phpversion();
		/*echo "<script type='text/javascript'>alert('<?php echo $username.$email.$fileversion; ?>');</script>";*/
